...
Runs the validation pipeline locally using Docker (same `jktz-survex` image as `/docker-exports`, built from `docker/Dockerfile.releaseExports`). Checks SRV file naming, invalid directives, compiles with cavern, and reports any unattached-station errors. Mirrors the Linux job in GitHub Actions `validate.yml`.
...
If missing, build it (first time takes several minutes; subsequent builds are near-instant due to layer caching):
