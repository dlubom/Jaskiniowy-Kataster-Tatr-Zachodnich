...
echo ""
bash scripts/_legacy/exports/exports.sh "${VERSION}" "${OUTDIR}" 2>&1 | sed 's/^/                   /'

echo ""
echo "Checking for empty .shp files..."
echo ""
bash scripts/_legacy/validation/check-empty-shapefiles.sh "${OUTDIR}"

echo ""
echo "Checking shapefile feature-count consistency (all vs sum-of-caves)..."
echo ""
bash scripts/_legacy/validation/check-shapefiles-count.sh "${VERSION}" "${OUTDIR}"

echo ""
echo "Checking shapefile geographic extent (Tatras only)..."
echo ""
bash scripts/_legacy/validation/check-shapefiles-extent.sh "${VERSION}" "${OUTDIR}"
